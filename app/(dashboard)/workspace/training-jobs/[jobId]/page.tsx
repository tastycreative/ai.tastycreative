  ...
  useEffect(() => {
    ...
    const updateMetrics = () => {
      ...
      const baseMetrics: TrainingMetrics = {
        ...
        currentPhase: job.status === "PROCESSING" ? "Training" : 
                     job.status === "INITIALIZING" ? "Initializing" : 
                     job.status === "SAMPLING" ? "Generating Samples" :
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              >
                <RefreshCw className={`w-4 h-4 mr-2 inline ${autoRefresh ? 'animate-spin' : ''}`} />
                Auto Refresh
              </button>
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/20 p-6 shadow-lg">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{metrics.currentStep}/{metrics.totalSteps}</span>
                </div>
                ...
                <div className="text-2xl font-bold text-gray-900">{metrics.progressPercentage}%</div>
              </div>
            </div>
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/20 p-6 shadow-lg">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-green-600">{metrics.loss.toFixed(4)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Learning Rate</span>
                  <span className="font-medium">{metrics.learningRate.toExponential(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Step Time</span>
                  <span className="font-medium">{metrics.stepTimeAvg.toFixed(2)}s/it</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/20 p-6 shadow-lg">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{metrics.memoryUsage?.toFixed(0)}%</span>
                </div>
                <div className="flex justify-between text-sm">
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/20 p-6 shadow-lg">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{formatDuration(metrics.timeElapsed)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Remaining</span>
                  <span className="font-medium">{formatDuration(metrics.estimatedTimeRemaining)}</span>
                </div>
                <div className="text-sm text-gray-600">
                  Phase: <span className="font-medium text-gray-900">{metrics.currentPhase}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/20 shadow-lg mb-8">
          ...
          <div className="p-6">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{job.modelConfig?.type || "LoRA"}</span>
                      </div>
                      <div className="flex justify-between">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div>
                        <span className="text-gray-600 block text-sm">Training Prompt</span>
                        <span className="font-medium text-sm bg-gray-100 rounded-lg px-3 py-2 block mt-1">
                          {job.trainingConfig?.trigger_word || job.description || "Custom training prompt"}
                        </span>
                      </div>
            ...
            {selectedTab === "logs" && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-gray-900">Live Training Logs</h3>
                  <div className="text-sm text-gray-600">
                    {logs.length} messages • Auto-updating every {refreshInterval/1000}s
                  </div>
                </div>
                    ...
                    <div className="space-y-1">
                      {logs.map((log) => (
                        <div key={log.id} className="flex items-start space-x-3">
                          ...
                          <span className={`text-xs ${getLogLevelColor(log.level)} min-w-0 flex-shrink-0`}>
            ...
            {selectedTab === "samples" && (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">Generated Samples</h3>
                
                {samples.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">
                    <ImageIcon className="w-12 h-12 mx-auto mb-4 text-gray-300" />
                    <p>No samples generated yet</p>
                    <p className="text-sm">Samples will appear here as training progresses</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {samples.map((sample) => (
                      <div key={sample.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        />
                        <div className="p-4">
                          <div className="text-sm text-gray-600 mb-2">Step {sample.step}</div>
              ...
              <div className="space-y-6">
                ...
                {metrics && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <h4 className="font-medium text-gray-900">Training Progress</h4>
                      <div className="space-y-4">
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600">Current Step</span>
                          <span className="font-mono text-lg">{metrics.currentStep.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600">Total Steps</span>
                          <span className="font-mono text-lg">{metrics.totalSteps.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600">Loss</span>
                          <span className="font-mono text-lg text-green-600">{metrics.loss.toFixed(6)}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600">Learning Rate</span>
                          <span className="font-mono text-lg">{metrics.learningRate.toExponential(3)}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-mono text-lg">{metrics.gpuUtilization?.toFixed(1)}%</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600">Memory Usage</span>
                          <span className="font-mono text-lg">{metrics.memoryUsage?.toFixed(1)}%</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600">Temperature</span>
                          <span className="font-mono text-lg">{metrics.temperature?.toFixed(1)}°C</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-gray-600">Step Time Avg</span>
                          <span className="font-mono text-lg">{metrics.stepTimeAvg.toFixed(2)}s/it</span>
                        </div>
                      </div>
                    </div>
                  </div>
