...
export default function TrainingJobsPage() {
  ...
  const { data: trainingJobs = [], isLoading, refetch } = trpc.getUserTrainingJobs.useQuery();
  ...
  const filteredJobs = trainingJobs.filter(job => {
    if (selectedTab === 'active') {
      return ['PENDING', 'QUEUED', 'INITIALIZING', 'PROCESSING', 'SAMPLING', 'SAVING'].includes(job.status);
    }
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="flex items-center justify-center h-64">
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-600 dark:text-gray-400">Loading training jobs...</span>
          </div>
        </div>
      </div>
    );
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">LoRAs</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <span className="ml-2 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full text-xs">
                {tab === 'all' 
                  ...
                  : tab === 'active'
                    ...
                    : (trainingStats?.completedJobs || 0) + (trainingStats?.failedJobs || 0)
                }
              </span>
            </button>
          ))}
      ...
      {filteredJobs.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
            {selectedTab === 'all' ? 'No training jobs yet' : `No ${selectedTab} jobs`}
          </h3>
