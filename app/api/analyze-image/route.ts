...
• Emphasize candid, organic moments`
    ...
    try {
      ...
      const fallbackData = {
        cameraDistance: extractField(content, 'camera distance') || 'medium shot',
        pose: extractField(content, 'pose') || 'natural standing pose',
        location: extractField(content, 'location') || 'casual indoor setting',
        outfit: extractField(content, 'outfit') || 'casual clothing',
        facialExpression: extractField(content, 'facial expression') || 'natural expression',
