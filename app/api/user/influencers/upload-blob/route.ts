...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate unique ComfyUI filename with userId (like the working blob-complete version)
    const uniqueComfyUIFileName = `${userId}_${timestamp}_${baseName}${fileExtension}`;
    ...
    try {
      ...
      const uploadSuccess = await uploadLoRAToComfyUI(blob.url, uniqueComfyUIFileName);
      if (uploadSuccess) {
        ...
        await prisma.influencerLoRA.update({
          ...
          data: { 
            syncStatus: 'SYNCED',
            comfyUIPath: `loras/${uniqueComfyUIFileName}`
          },
        });
