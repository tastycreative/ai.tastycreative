...
export async function GET(request: NextRequest) {
  try {
    ...
    if (contentCreatorId) {
      ...
      const getDescendantFolderIds = (folderId: string, folders: typeof allFolders): Set<string> => {
      ...
      // Count items per folder for this creator (including nested subfolders)
      const folderItemCounts = new Map<string, number>();
      relevantFolders.forEach(folder => {
        // Get this folder's ID and all descendant folder IDs
        const descendantIds = getDescendantFolderIds(folder.id, allFolders);
        
        // Count items in this folder + all descendant folders
        const count = items.filter(item => {
          return item.folderId === folder.id || descendantIds.has(item.folderId);
        }).length;
        
        folderItemCounts.set(folder.id, count);
      });
