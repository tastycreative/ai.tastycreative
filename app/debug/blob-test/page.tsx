...
export default function BlobTestPage() {
  ...
  const testServerFileUpload = async () => {
    ...
    try {
      ...
      const testFile = new File([testContent], 'test-lora.safetensors', { type: 'application/octet-stream' });
  ...
  const testDirectUpload = async () => {
    ...
    try {
      ...
      const testFile = new File([testContent], 'test-direct.safetensors', { type: 'application/octet-stream' });
      
      const formData = new FormData();
      formData.append('file', testFile);
      formData.append('displayName', 'Test Direct Upload');
      formData.append('description', 'Test direct upload bypassing blob storage');
  ...
  const testClientUpload = async () => {
    ...
    try {
      ...
      const testFile = new File([testContent], 'client-test.txt', { type: 'text/plain' });
