...
const QueueItem = memo(function QueueItem({
  ...
  return (
    <div
      onClick={onSelect}
      className={`p-4 border-b border-brand-mid-pink/10 cursor-pointer transition-colors ${
        isSelected 
...
function QueuePanel({ queue, selectedTicket, onSelectTicket, searchQuery, onSearchChange }: QueuePanelProps) {
  const parentRef = useRef<HTMLDivElement>(null);
  
  // Virtual list for performance with large queues
  const virtualizer = useVirtualizer({
    count: queue.length,
    getScrollElement: () => parentRef.current,
    estimateSize: useCallback(() => 156, []), // Estimated height of each item
    overscan: 5,
  });
