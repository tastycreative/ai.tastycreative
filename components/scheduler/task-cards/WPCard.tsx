...
export function WPCard({ task, team, onUpdate, onDelete, compact, schedulerToday, weekStart, profileName, autoOpen, onModalOpen, onModalClose, streak }: TaskCardProps) {
  ...
  // ── Compact ──
  if (compact) {
    const label = fields.postSchedule || fields.type || task.taskName || '';
    return (
      <>
        <div
      ...
      <div
        ...
        className={`flex flex-col gap-1.5 rounded-lg border p-2.5 cursor-pointer transition-colors ${
          isFlagged
            ...
            : 'bg-white dark:bg-[#0c0c1a] border-gray-200 dark:border-[#111124] hover:bg-sky-50/40 dark:hover:bg-sky-950/10'
        }`}
