      ...
      <div className="w-10 sm:w-12 h-10 sm:h-12 flex-shrink-0 rounded-lg overflow-hidden bg-zinc-100 dark:bg-zinc-800">
        {item.fileType.startsWith("image/") ? (
          ...
          <div className="relative w-full h-full bg-zinc-100 dark:bg-zinc-800">
            <LazyVideoThumbnail src={item.awsS3Url} className="w-full h-full" />
            <div className="absolute inset-0 flex items-center justify-center bg-black/20">
...
interface FolderTreeItemProps {
  ...
  onContextMenu: (folder: VaultFolder, e: React.MouseEvent) => void;
...
const FolderTreeItem = memo(function FolderTreeItem({
  ...
  const subfolders = allFolders
    ...
    .sort((a, b) => {
      ...
      if (!a.isDefault && b.isDefault) return 1;
      // Then by custom order, or alphabetical if order not set
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent) => {
      if (folder.isDefault) return; // Don't allow dragging default folders
      e.dataTransfer.setData("text/vault-folder-id", folder.id);
      e.dataTransfer.setData(
        "text/vault-folder-parent",
        folder.parentId || "root",
      );
      e.dataTransfer.setData("text/vault-folder-profile", folder.profileId);
      e.dataTransfer.effectAllowed = "move";
      // Store metadata in ref for access during dragOver
      draggedFolderRef.current = {
        id: folder.id,
  ...
  const handleDragEnd = useCallback(() => {
    // Cleanup ref when drag operation ends (whether dropped or cancelled)
    draggedFolderRef.current = null;
  }, [draggedFolderRef]);

  const handleDragOver = useCallback(
    (e: React.DragEvent) => {
      const draggedFolderId = e.dataTransfer.types.includes(
        "text/vault-folder-id",
      );
      if (!draggedFolderId || !draggedFolderRef.current) return;

      e.preventDefault();
      e.stopPropagation();

      const draggedData = draggedFolderRef.current;
      const draggedParent = draggedData.parentId || "root";
      const currentParent = folder.parentId || "root";
      const draggedProfile = draggedData.profileId;
      ...
      // Same level reordering (before/after) if same parent and profile
      if (
        draggedParent === currentParent &&
        draggedProfile === folder.profileId &&
        !folder.isDefault
      ) {
        ...
        // Different level - moving into folder
        setDropPosition("inside");
        e.dataTransfer.dropEffect = "move";
      }
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const draggedParent =
        e.dataTransfer.getData("text/vault-folder-parent") || "root";
      const currentParent = folder.parentId || "root";
      const draggedProfile = e.dataTransfer.getData(
        "text/vault-folder-profile",
      );

      if (!sourceFolderId || sourceFolderId === folder.id) {
        setDropPosition(null);
        draggedFolderRef.current = null; // Cleanup
        return;
      }
      ...
      if (
        ...
        // Call special reorder with target
        const customEvent = new CustomEvent("folder-reorder", {
          detail: {
            sourceFolderId,
      ...
      } else if (dropPosition === "inside" && onDropFolder) {
        // Move into folder (nesting)
  ...
  const itemCount = folder.isDefault
    ...
    : getRecursiveItemCount(folder.id, allFolders, vaultItems);
  ...
  return (
    <div>
      ...
      {isDragOver && dropPosition === "before" && (
        <div className="h-0.5 bg-[#5DC3F8] rounded-full mx-2 mb-1 animate-pulse" />
      )}
      ...
      {editingFolderId === folder.id ? (
        ...
        >
          ...
          <button onClick={onCancelEdit} className="p-1.5 text-header-muted">
            <X className="w-4 h-4" />
          </button>
        </div>
      ) : (
        <div
          className={`group w-full flex items-center gap-2 py-2.5 rounded-xl transition-all cursor-pointer relative ${
            isDragOver
              ? "bg-[#5DC3F8]/20 text-[#5DC3F8] dark:text-[#5DC3F8] border-2 border-dashed border-[#5DC3F8]/50 ring-1 ring-[#5DC3F8]/30"
              : isActive
                ? "bg-gradient-to-r from-[#EC67A1]/20 to-[#F774B9]/20 text-[#E1518E] dark:text-[#F774B9] border border-[#EC67A1]/30"
                : "hover:bg-zinc-100 dark:hover:bg-zinc-800/50 text-sidebar-foreground border border-transparent"
          }`}
        ...
        >
          ...
          {/* Right side container for count and actions */}
          <div className="flex items-center gap-1 flex-shrink-0 ml-auto">
            {/* Item count - Always visible */}
            <span
              className={`text-xs px-2 py-0.5 rounded-full transition-opacity ${isActive ? "bg-[#EC67A1]/30 text-[#E1518E] dark:text-[#F774B9]" : "bg-zinc-100 dark:bg-zinc-800 text-header-muted"} group-hover:opacity-0`}
            >
              {itemCount}
            </span>

            {/* Action buttons - Show on hover, overlaying the count */}
            <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 transition-opacity absolute right-2">
              {!folder.isDefault && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg bg-white dark:bg-[#1a1625] shadow-sm"
      ...
      {isDragOver && dropPosition === "after" && (
        <div className="h-0.5 bg-[#5DC3F8] rounded-full mx-2 mt-1 mb-1 animate-pulse" />
      )}
...
export function VaultContent() {
  ...
  const [moveFolderDestinationId, setMoveFolderDestinationId] = useState<
    string | null
  >(null);
  ...
  const [showBulkMoveFoldersModal, setShowBulkMoveFoldersModal] =
    useState(false);
  ...
  const [selectedUserToShare, setSelectedUserToShare] =
    useState<AvailableUser | null>(null);
  ...
  const [referenceImagePopup, setReferenceImagePopup] = useState<string | null>(
    null,
  );
  ...
  const [googleDriveFolders, setGoogleDriveFolders] = useState<
    GoogleDriveFolder[]
  >([]);
  const [googleDriveBreadcrumbs, setGoogleDriveBreadcrumbs] = useState<
    GoogleDriveBreadcrumb[]
  >([{ id: null, name: "My Drive" }]);
  const [currentGoogleDriveFolderId, setCurrentGoogleDriveFolderId] = useState<
    string | null
  >(null);
  const [selectedGoogleDriveFiles, setSelectedGoogleDriveFiles] = useState<
    Set<string>
  >(new Set());
  const [loadingGoogleDriveFiles, setLoadingGoogleDriveFiles] = useState(false);
  const [importingFromGoogleDrive, setImportingFromGoogleDrive] =
    useState(false);
  const [googleDriveImportSuccess, setGoogleDriveImportSuccess] = useState<{
    itemCount: number;
  } | null>(null);
  ...
  const [sortBy, setSortBy] = useState<"date" | "size" | "name" | "type">(
    "date",
  );
  ...
  // Duplicate Detection
  const [duplicates, setDuplicates] = useState<Map<string, string[]>>(
    new Map(),
  );
  ...
  const addToRecentlyViewed = useCallback((itemId: string) => {
    setRecentlyViewed((prev) => {
      const newRecent = [itemId, ...prev.filter((id) => id !== itemId)].slice(
        0,
        20,
      );
    ...
    (items: VaultItem[], query: string) => {
      ...
      return items.filter((item) => {
        ...
        if (searchInMetadata && item.metadata) {
          if (item.metadata.prompt && fuzzyMatch(query, item.metadata.prompt))
            return true;
          if (item.metadata.model && fuzzyMatch(query, item.metadata.model))
            return true;
          if (item.metadata.source && fuzzyMatch(query, item.metadata.source))
            return true;
          if (
            item.metadata.generationType &&
            fuzzyMatch(query, item.metadata.generationType)
          )
            return true;
        }
  ...
  const handleSelectItem = useCallback(
    (itemId: string, e?: React.MouseEvent, itemList?: VaultItem[]) => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const deletePromises = Array.from(selectedItems).map(async (id) => {
        ...
        return { id, success: response.ok, status: response.status, response };
      });
  ...
  const handleDownloadZip = async () => {
    ...
    try {
      ...
      const selectedFiles = itemsSource.filter((item) =>
      ...
      if (successCount === 0) {
        throw new Error(
          "Failed to download any files. Please check your connection and try again.",
        );
      }
      ...
      if (failCount > 0) {
        showToast(
          `Downloaded ${successCount} of ${selectedFiles.length} file(s)`,
          "info",
        );
      } else {
  ...
  const connectToGoogleDrive = async () => {
    try {
      ...
      const response = await fetch(
        `/api/auth/google?redirect=${encodeURIComponent(currentPath)}`,
      );
      const data = await response.json();
      if (data.authUrl) {
  ...
  const fetchGoogleDriveContents = async (folderId: string | null = null) => {
    ...
    try {
      ...
      if (data.authError) {
        ...
        setGoogleDriveError(
          "Session expired. Please reconnect to Google Drive.",
        );
        return;
      }
      ...
      if (data.error) {
        if (
          ...
          setGoogleDriveError(
            "Unable to access this folder. You may not have permission or the link may be invalid.",
          );
        } else {
  ...
  const navigateToGoogleDriveFolder = (folder: GoogleDriveFolder) => {
    ...
    setGoogleDriveBreadcrumbs((prev) => [
      ...prev,
  ...
  const browseGoogleDriveLink = async () => {
    ...
    if (!folderId) {
      setGoogleDriveError(
        "Invalid Google Drive link. Please paste a valid folder link.",
      );
      return;
    }
    ...
    try {
      ...
      setGoogleDriveError(
        "Failed to access the linked folder. Make sure you have permission.",
      );
    }
  };
  ...
  const openGoogleDriveModal = () => {
    if (!selectedFolderId || !selectedProfileId || selectedProfileId === "all")
      return;
  ...
  const selectAllGoogleDriveFiles = () => {
    if (selectedGoogleDriveFiles.size === googleDriveFiles.length) {
      ...
      setSelectedGoogleDriveFiles(
        new Set(googleDriveFiles.map((file) => file.id)),
      );
    }
  };
  ...
  const importFromGoogleDrive = async () => {
    ...
    try {
      ...
      showToast(
        `Imported ${data.itemCount} file(s) from Google Drive`,
        "success",
      );
  ...
  const loadCurrentShares = async (folderId: string) => {
    try {
      const response = await fetch(
        `/api/vault/folders/share?vaultFolderId=${folderId}`,
      );
  ...
  const handleOpenShareModal = async (folder: VaultFolder) => {
    ...
    await Promise.all([loadCurrentShares(folder.id), loadAvailableUsers()]);
  ...
  const getDescendantFolderIds = useCallback(
    (folderId: string, folderList: VaultFolder[]): Set<string> => {
  ...
  // Helper function to count items in a folder and all its descendants
  const getRecursiveItemCount = useCallback(
    (
      folderId: string,
      folderList: VaultFolder[],
      items: VaultItem[],
      ...
      const descendantIds = getDescendantFolderIds(folderId, folderList);

      // Count items in this folder + all descendant folders
      return items.filter((item) => {
        return item.folderId === folderId || descendantIds.has(item.folderId);
      }).length;
    },
  ...
  const handleMoveFolder = useCallback(
    async (sourceFolderId: string, destinationFolderId: string | null) => {
      const source = [...folders, ...allFolders].find(
        (f) => f.id === sourceFolderId,
      );
      if (!source || source.isDefault) return;

      // Prevent moving into default "All Media" folder
      if (destinationFolderId) {
        const destination = [...folders, ...allFolders].find(
          (f) => f.id === destinationFolderId,
        );
        if (destination?.isDefault) {
          showToast("Cannot move folders into All Media folder", "error");
          return;
        }
      }
  ...
  const handleRootDropZoneDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const sourceFolderId = e.dataTransfer.getData("text/vault-folder-id");
      if (sourceFolderId) {
        handleMoveFolder(sourceFolderId, null); // null = root level
      }
    },
    [handleMoveFolder],
  );

  // Folder reordering handler
  const handleReorderFolder = useCallback(
    async (folderId: string, direction: "up" | "down") => {
      const folder = [...folders, ...allFolders].find((f) => f.id === folderId);
      ...
      // Update orders
      const updatedOrders = siblings.map((f, idx) => {
        let newOrder = idx * 10; // Give 10-unit spacing
        if (idx === currentIndex) {
          newOrder = targetIndex * 10;
        } else if (idx === targetIndex) {
          newOrder = currentIndex * 10;
        }
        return { folderId: f.id, order: newOrder };
      });
  ...
  // Drag-and-drop reordering handler (when dropping before/after a folder)
  const handleDragDropReorder = useCallback(
    async (
      sourceFolderId: string,
      targetFolderId: string,
      position: "before" | "after",
    ) => {
      const sourceFolder = [...folders, ...allFolders].find(
        (f) => f.id === sourceFolderId,
      );
      const targetFolder = [...folders, ...allFolders].find(
      ...
      const siblings = uniqueFolders
        .filter(
          (f) =>
            ...
            f.profileId === sourceFolder.profileId &&
            !f.isDefault,
        )
        .sort((a, b) => {
          const orderA = a.order ?? 999999;
      ...
      // Assign new orders
      const updatedOrders = reordered.map((f, idx) => ({
        folderId: f.id,
        order: idx * 10,
      }));

      // Validate that all folders in updatedOrders exist in our current state
      const invalidFolders = updatedOrders.filter(
        (u) => !allFoldersMap.has(u.folderId),
      );
      if (invalidFolders.length > 0) {
        console.error(
          "Trying to update folders that don't exist in state:",
          invalidFolders,
        );
        return;
      }
  ...
  // Listen for custom drag-drop reorder events
  useEffect(() => {
    const handleReorderEvent = (e: Event) => {
      const customEvent = e as CustomEvent;
      const { sourceFolderId, targetFolderId, position } = customEvent.detail;
      handleDragDropReorder(sourceFolderId, targetFolderId, position);
    };

    window.addEventListener("folder-reorder", handleReorderEvent);
    return () =>
      window.removeEventListener("folder-reorder", handleReorderEvent);
  ...
  const handleContextMenu = useCallback(
    (folder: VaultFolder, e: React.MouseEvent) => {
      e.preventDefault();
      e.stopPropagation();
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ error: "Failed to update folder" }));
        console.error(
          "Folder update failed with status:",
          response.status,
          errorData,
        );
    ...
    try {
      ...
      if (selectedFolderId === folderId) {
        const remaining = folders.filter(
          (f) => f.id !== folderId && f.profileId === selectedProfileId,
        );
      ...
      const uploadPromises = newFiles.map(async (file) => {
        ...
        if (!presignedResponse.ok) {
          const errorData = await presignedResponse
            ...
            .catch(() => ({ error: "Unknown error" }));
          throw new Error(
            errorData.error || `Failed to get upload URL for ${file.name}`,
          );
        }

        const { presignedUrl, s3Key, awsS3Url, profileId, folderId } =
        ...
        if (!confirmResponse.ok) {
          const errorData = await confirmResponse
            ...
            .catch(() => ({ error: "Unknown error" }));
          throw new Error(
            errorData.error || `Failed to confirm upload for ${file.name}`,
          );
        }
  ...
  const handleReuseInSeeDreamI2I = (item: VaultItem) => {
    ...
    router.push(
      `/${tenant}/workspace/generate-content/seedream-image-to-image`,
    );
  };
  ...
  const handleReuseInFluxStyleTransfer = (item: VaultItem) => {
    ...
    sessionStorage.setItem(
      "flux-style-transfer-reuse",
      JSON.stringify(reuseData),
    );
  ...
  const handleReuseInSeeDreamI2V = (item: VaultItem) => {
    ...
    router.push(
      `/${tenant}/workspace/generate-content/seedream-image-to-video`,
    );
  };
  ...
  const handleReuseInKlingMultiI2V = (item: VaultItem) => {
    ...
    router.push(
      `/${tenant}/workspace/generate-content/kling-multi-image-to-video`,
    );
  };
  ...
  const allFilteredItems = useMemo(() => {
    ...
    const getExportBatchKey = (item: VaultItem): string | null => {
      if (
        item.metadata?.source === "sexting-set-export" &&
        item.metadata?.originalSetId
      ) {
    ...
    baseItems = baseItems.filter((item) => {
      ...
      if (contentFilter === "photos")
        return (
          item.fileType.startsWith("image/") && item.fileType !== "image/gif"
        );
  ...
  const allFilteredCreatorItems = useMemo(() => {
    ...
    const selectedFolder = creatorFolders.find(
      (f) => f.id === selectedCreatorFolderId,
    );
    ...
    return contentCreatorItems
      .filter((item) => {
        ...
        if (
          ...
          item.creatorId !== selectedContentCreator.id
        ) {
          return false;
        }
        return true;
  ...
  const selectedFolder = selectedSharedFolder
    ? {
        id: selectedSharedFolder.folderId,
        name: selectedSharedFolder.folderName,
        profileId: selectedSharedFolder.profileId,
        isDefault: selectedSharedFolder.isDefault,
      }
    ...
    <>
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl shadow-2xl backdrop-blur-xl ${
                toast.type === "success"
                  ...
                  : toast.type === "error"
                    ...
                    : "bg-[#EC67A1]/20 text-[#F774B9] border border-[#EC67A1]/30"
              }`}
            >
              {toast.type === "success" && (
              ...
              {toast.type === "info" && (
                <AlertCircle className="w-5 h-5 text-[#EC67A1]" />
              )}
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 animate-fadeIn">
            ...
            <div className="relative bg-white dark:bg-[#1a1625] rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg animate-slideUp max-h-[90vh] sm:max-h-none overflow-hidden border border-[#EC67A1]/20">
              <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[#EC67A1]/10">
                <h3 className="text-base sm:text-lg font-semibold text-sidebar-foreground">
                  Upload Files
                </h3>
                <button
                  onClick={() => {
              ...
              <div className="p-4 sm:p-5 space-y-4 overflow-y-auto max-h-[60vh] sm:max-h-none vault-scroll">
                ...
                >
                  <div className="flex flex-col items-center gap-3">
                    <div
                      className={`p-3 rounded-full ${isDragging ? "bg-[#5DC3F8]/20" : "bg-zinc-100 dark:bg-zinc-800"}`}
                    >
                      <Upload
                        className={`w-6 h-6 ${isDragging ? "text-[#5DC3F8]" : "text-header-muted"}`}
                      />
                    </div>
                    <div className="text-center">
                      ...
                      <p className="text-xs text-header-muted mt-1 hidden sm:block">
                        or drag and drop
                      </p>
                    </div>
                    <input
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="max-h-40 overflow-y-auto space-y-2 vault-scroll">
                      {newFiles.map((file, index) => (
                        <div
                          key={index}
                          className="flex items-center gap-3 p-2 bg-zinc-100 dark:bg-zinc-800 rounded-lg"
                        >
                          <div className="p-2 bg-zinc-200 dark:bg-zinc-700 rounded-lg">
                            {file.type.startsWith("image/") ? (
                              <ImageIcon className="w-4 h-4 text-[#5DC3F8]" />
                            ) : file.type.startsWith("video/") ? (
                              <VideoIcon className="w-4 h-4 text-purple-500 dark:text-purple-400" />
                            ) : file.type.startsWith("audio/") ? (
                              <Music4 className="w-4 h-4 text-[#EC67A1]" />
                            ) : (
                              <FileIcon className="w-4 h-4 text-header-muted" />
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                ...
                {uploadProgress > 0 && (
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-[#5DC3F8] font-medium">
                        {uploadProgress}%
                      </span>
                    </div>
                    <div className="h-2 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[#5DC3F8] rounded-full transition-all"
              ...
              <div className="flex gap-3 p-4 sm:p-5 border-t border-[#EC67A1]/10">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 text-sidebar-foreground bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-lg text-sm sm:text-base font-medium transition-colors disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 px-4 py-2.5 text-white bg-[#EC67A1] hover:bg-[#E1518E] rounded-lg text-sm sm:text-base font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {uploadProgress > 0 ? (
        ...
        createPortal(
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 animate-fadeIn">
            ...
            <div className="relative bg-white dark:bg-[#1a1625] rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md animate-slideUp border-t sm:border border-[#EC67A1]/20">
              <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[#EC67A1]/10">
                <h3 className="text-base sm:text-lg font-semibold text-sidebar-foreground">
                  {isCopyMode ? "Copy" : "Move"} {selectedItems.size} item(s)
                </h3>
                <button
                  onClick={() => {
                    setShowMoveModal(false);
              ...
              <div className="p-4 sm:p-5 space-y-4">
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 px-3 sm:px-4 py-2.5 rounded-lg text-sm sm:text-base font-medium transition-colors ${!isCopyMode ? "bg-[#5DC3F8] text-white" : "bg-zinc-100 dark:bg-zinc-800 text-sidebar-foreground hover:bg-zinc-200 dark:hover:bg-zinc-700"}`}
                  >
                    <Move className="w-4 h-4" /> Move
                  </button>
                  <button
                    onClick={() => setIsCopyMode(true)}
                    className={`flex-1 flex items-center justify-center gap-2 px-3 sm:px-4 py-2.5 rounded-lg text-sm sm:text-base font-medium transition-colors ${isCopyMode ? "bg-emerald-600 text-white" : "bg-zinc-100 dark:bg-zinc-800 text-sidebar-foreground hover:bg-zinc-200 dark:hover:bg-zinc-700"}`}
                  >
                    ...
                    {(() => {
                      ...
                      const renderFolderTree = (
                        ...
                        const children = foldersForProfile
                          .filter(
                            (f) => f.parentId === parentId && !f.isDefault,
                          )
                        ...
                        return children.flatMap((folder) => {
                          ...
                          return [
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                !isExcluded && setMoveToFolderId(folder.id)
                              }
                              disabled={isExcluded}
                          ...
                          {isViewingShared ? (
                            <>
                              {sortedProfiles.map((profile) => {
                                const profileFolders = allFolders.filter(
                                  (f) => f.profileId === profile.id,
                                );
                                const hasVisibleFolders = profileFolders.some(
                                  (f) => !f.isDefault,
                                );
                            ...
                            <>
                              {sortedProfiles.map((profile) => {
                                ...
                                const visibleNonDefault = profileFolders.filter(
                                  (f) => !f.isDefault && !excludeIds.has(f.id),
                                );
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={async () => {
                      if (!moveToFolderId) {
                        showToast("Select a destination folder", "error");
                        return;
                      }
                      setIsMoving(true);
                      try {
                        ...
                        if (isCopyMode) {
                          ...
                          if (failedCopies.length > 0)
                            showToast(
                              `Failed to copy ${failedCopies.length} item(s)`,
                              "error",
                            );
                          else
                            showToast(
                          ...
                          if (failedMoves.length > 0)
                            showToast(
                              `Failed to move ${failedMoves.length} item(s)`,
                              "error",
                            );
                          else
                            showToast(
                        ...
                        showToast(
                          `Failed to ${isCopyMode ? "copy" : "move"} items`,
                          "error",
                        );
                      } finally {
                        setIsMoving(false);
              ...
              <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[#EC67A1]/10">
                <div>
                  ...
                  <p className="text-xs text-header-muted mt-0.5">
                    Moving &ldquo;{folderToMove.name}&rdquo;
                  </p>
                </div>
                <button
                  onClick={() => {
                        ...
                        return (
                          <div key={folder.id}>
                            <button
                              onClick={() =>
                                !isInvalid &&
                                setMoveFolderDestinationId(folder.id)
                              }
                              disabled={isInvalid}
                            ...
                            >
                              {isSelected ? (
                                <FolderOpen className="w-4 h-4 flex-shrink-0 text-[#5DC3F8]" />
                              ) : (
                                <FolderClosed className="w-4 h-4 flex-shrink-0" />
                              )}
                              <span className="truncate">{folder.name}</span>
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 animate-fadeIn">
            ...
            <div className="relative bg-white dark:bg-[#1a1625] rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md animate-slideUp border-t sm:border border-[#EC67A1]/20">
              <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[#EC67A1]/10">
                <div>
                  <h3 className="text-base sm:text-lg font-semibold text-sidebar-foreground">
                    Move {selectedFolderIds.size} Folder(s)
                  </h3>
                  <p className="text-xs text-header-muted mt-0.5">
                    Select destination for the selected folders
                  </p>
                </div>
                <button
                  onClick={() => {
                        ...
                        return (
                          <div key={folder.id}>
                            <button
                              onClick={() =>
                                !isInvalid &&
                                setBulkMoveFolderDestinationId(folder.id)
                              }
                              disabled={isInvalid}
                              className={`w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors ${
                                isInvalid
        ...
        createPortal(
          ...
          >
            <button
              ...
              className="absolute top-3 sm:top-4 right-3 sm:right-4 p-2 bg-white/10 hover:bg-white/20 rounded-full transition-colors z-10"
            >
              ...
              >
                {previewItem.fileType.startsWith("image/") ? (
                  ...
                  <video
                    ...
                    autoPlay
                    playsInline
                    className="max-w-full max-h-[75vh] sm:max-h-[85vh] rounded-lg"
                  />
                  ...
                  <div className="bg-white dark:bg-[#1a1625] rounded-2xl p-6 sm:p-8 text-center border border-[#EC67A1]/20 mx-4">
                    ...
                    <p className="text-sidebar-foreground font-medium mb-4 text-sm sm:text-base truncate max-w-[250px] sm:max-w-none mx-auto">
                  ...
                  <div className="bg-white dark:bg-[#1a1625] rounded-2xl p-6 sm:p-8 text-center border border-[#EC67A1]/20 mx-4">
                    <FileIcon className="w-16 sm:w-20 h-16 sm:h-20 text-header-muted mx-auto mb-4" />
                    <p className="text-header-muted text-sm sm:text-base">
                ...
                <div className="hidden sm:block w-80 bg-white/95 dark:bg-[#1a1625]/95 backdrop-blur border border-[#EC67A1]/20 rounded-2xl p-5 overflow-y-auto max-h-[85vh] animate-slideUp">
                  ...
                  <div className="space-y-4">
                    ...
                    {previewItem.metadata.generatedByName && (
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-header-muted uppercase tracking-wider">
                          Generated By
                        </label>
                    ...
                    {(previewItem.metadata.source ||
                      previewItem.metadata.generationType) && (
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-header-muted uppercase tracking-wider">
                          Source
                        </label>
                    ...
                    {(() => {
                      ...
                      if (previewItem.metadata.referenceImageUrl)
                        refImages.push(previewItem.metadata.referenceImageUrl);
                      if (
                        previewItem.metadata.imageUrl &&
                        !refImages.includes(previewItem.metadata.imageUrl)
                      )
                        refImages.push(previewItem.metadata.imageUrl);
                      if (previewItem.metadata.referenceImageUrls)
                        refImages.push(
                          ...previewItem.metadata.referenceImageUrls.filter(
                            (url: string) => !refImages.includes(url),
                          ),
                        );
                      if (previewItem.metadata.sourceImageUrls)
                        refImages.push(
                          ...previewItem.metadata.sourceImageUrls.filter(
                            (url: string) => !refImages.includes(url),
                          ),
                        );
                      ...
                      const displayedImages = showAllReferenceImages
                        ? refImages
                        : refImages.slice(0, 4);
                      ...
                      return (
                        <div className="space-y-2">
                          <label className="text-xs font-medium text-header-muted uppercase tracking-wider">
                            Reference Image{refImages.length > 1 ? "s" : ""}
                          </label>
            ...
            <div
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <span className="text-sidebar-foreground text-xs sm:text-sm truncate max-w-[100px] sm:max-w-[200px]">
                {previewItem.fileName}
              </span>
              <span className="text-header-muted text-xs sm:text-sm hidden sm:inline">
              ...
              <button
                ...
                title="Download"
              >
                <Download className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-white" />
              </button>
              {canEdit && (
                <button
                  onClick={() => {
                  ...
                  className="p-1.5 sm:p-2 bg-red-500/20 hover:bg-red-500/30 rounded-full transition-colors"
                >
                  <Trash2 className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-red-500 dark:text-red-400" />
                </button>
              )}
            </div>
          </div>,
            ...
            <div className="relative bg-white dark:bg-[#1a1625] rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg max-h-[85vh] sm:max-h-[90vh] overflow-hidden animate-slideUp border-t sm:border border-[#EC67A1]/20">
              <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[#EC67A1]/10">
                ...
                <button
                  ...
                  className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors flex-shrink-0"
                >
                  <X className="w-5 h-5 text-header-muted" />
                </button>
              </div>
                  ...
                  <>
                    <div className="space-y-3">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-10 pr-4 py-2.5 bg-zinc-100 dark:bg-zinc-800 border border-[#EC67A1]/20 rounded-lg text-sidebar-foreground placeholder-header-muted focus:ring-2 focus:ring-[#EC67A1] focus:border-[#EC67A1]"
                        />
                      </div>
                      <div className="max-h-32 overflow-y-auto border border-[#EC67A1]/20 rounded-lg vault-scroll">
                        {availableUsers
                          ...
                          .filter(
                            (user) =>
                              !currentShares.some(
                                (s) => s.sharedWithClerkId === user.clerkId,
                              ),
                          )
                          .slice(0, 10)
                          .map((user) => (
                            <button
                              ...
                              className={`w-full flex items-center gap-3 px-3 py-2 text-left transition-colors ${selectedUserToShare?.clerkId === user.clerkId ? "bg-[#5DC3F8]/20" : "hover:bg-zinc-100 dark:hover:bg-zinc-800"}`}
                            >
                              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#5DC3F8] to-[#EC67A1] flex items-center justify-center text-white text-sm font-medium flex-shrink-0">
                              ...
                              {selectedUserToShare?.clerkId ===
                                user.clerkId && (
                                <Check className="w-5 h-5 text-[#5DC3F8] flex-shrink-0" />
                              )}
                            </button>
                          ))}
                        {availableUsers.length === 0 && (
                          <p className="text-center py-4 text-header-muted text-sm">
                            No users available
                          </p>
                        )}
                      </div>
                        ...
                        <>
                          <div className="flex gap-2">
                            <button
                              ...
                              className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${sharePermission === "VIEW" ? "bg-[#5DC3F8]/20 text-[#5DC3F8] border border-[#5DC3F8]" : "bg-zinc-100 dark:bg-zinc-800 text-header-muted hover:bg-zinc-200 dark:hover:bg-zinc-700"}`}
                            >
                              <Eye className="w-4 h-4" /> View only
                            </button>
                            <button
                              onClick={() => setSharePermission("EDIT")}
                              className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${sharePermission === "EDIT" ? "bg-emerald-600/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500" : "bg-zinc-100 dark:bg-zinc-800 text-header-muted hover:bg-zinc-200 dark:hover:bg-zinc-700"}`}
                            >
                              <Edit2 className="w-4 h-4" /> Can edit
                            </button>
                          </div>
                          <input
                            type="text"
                            ...
                            className="w-full px-4 py-2.5 bg-zinc-100 dark:bg-zinc-800 border border-[#EC67A1]/20 rounded-lg text-sidebar-foreground placeholder-header-muted focus:ring-2 focus:ring-[#EC67A1] focus:border-[#EC67A1]"
                          />
                          <button
                            onClick={handleShareFolder}
                            disabled={shareModalLoading}
                            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[#EC67A1] hover:bg-[#E1518E] text-white rounded-lg font-medium transition-colors disabled:opacity-50"
                          >
                            {shareModalLoading ? (
                              <Loader2 className="w-4 h-4 animate-spin" />
                            ) : (
                    ...
                    <div className="pt-4 border-t border-[#EC67A1]/10">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-xs bg-zinc-100 dark:bg-zinc-800 text-header-muted px-2 py-1 rounded-full">
                          {currentShares.length}
                        </span>
                      </div>
                      {currentShares.length === 0 ? (
                        <div className="text-center py-6 border border-dashed border-[#EC67A1]/20 rounded-lg">
                          <Users className="w-8 h-8 text-header-muted mx-auto mb-2" />
                          ...
                          {currentShares.map((share) => (
                            ...
                            >
                              <div className="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  {share.sharedWithUser?.email && (
                                    <p
                                      className="text-xs text-header-muted truncate"
                                      title={share.sharedWithUser.email}
                                    >
                                      {share.sharedWithUser.email}
                                    </p>
                                  )}
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                <span
                                  className={`text-xs px-2 py-1 rounded-full ${share.permission === "EDIT" ? "bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400" : "bg-[#5DC3F8]/20 text-[#5DC3F8]"}`}
                                >
                                  {share.permission}
                                </span>
                                <button
      ...
      <div className="h-[calc(100vh-120px)] sm:h-[calc(100vh-120px)] flex bg-white dark:bg-[#0a0a0f] rounded-2xl border border-[#EC67A1]/10 dark:border-[#EC67A1]/20 shadow-lg overflow-hidden relative">
        ...
        >
          ...
          {isAdmin && adminViewMode === "creators" && (
            <div className="px-4 py-3 border-b border-[#EC67A1]/10">
              <label className="text-xs font-semibold text-header-muted uppercase tracking-wider mb-2 block">
                Content Creators
              </label>
              <select
              ...
              >
                <option
                  value="all"
                  className="bg-white dark:bg-zinc-800 text-sidebar-foreground"
                >
                  All Content Creators ({contentCreators.length})
                </option>
                ...
                {isAllProfiles ? (
                  <>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[10px] text-header-muted">
          ...
          <div className="flex-1 overflow-y-auto vault-scroll p-4">
            {adminViewMode === "personal" && (
              <>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-semibold text-header-muted uppercase tracking-wider">
                    Folders
                  </span>
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      disabled={!selectedProfileId || isAllProfiles}
                      className="p-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors disabled:opacity-50"
                      title={
                        isAllProfiles
                          ? "Select a specific profile to create folders"
                          : undefined
                      }
                    >
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        autoFocus
                        className="flex-1 px-3 py-2 text-sm bg-zinc-100 dark:bg-zinc-800 border border-[#EC67A1]/20 rounded-xl text-sidebar-foreground placeholder-header-muted focus:ring-2 focus:ring-[#EC67A1] focus:border-[#EC67A1]"
                      />
                      <button
                        onClick={handleCreateFolder}
                        disabled={!folderNameInput.trim()}
                        className="p-2 bg-gradient-to-r from-[#EC67A1] to-[#F774B9] text-white rounded-xl disabled:opacity-50"
                      >
                        <Check className="w-4 h-4" />
                      </button>
                      <button
                        ...
                        className="p-2 bg-zinc-200 dark:bg-zinc-700 text-header-muted rounded-xl"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    ...
                    ? // Group folders by profile - owned profiles first, then shared
                      (() => {
                        ...
                        for (const profile of profiles) {
                          const profileFolders = visibleFolders.filter(
                            (f) => f.profileId === profile.id,
                          );
                          if (profileFolders.length === 0) continue;

                          // Check if any folder for this profile is owned by current user
                          const isOwned = profileFolders.some(
                            (f) => f.isOwnedProfile === true,
                          );
                          if (isOwned) {
                          ...
                          return (
                            <div key={profile.id} className="mb-3">
                              <div className="flex items-center gap-2 px-3 py-1.5 text-xs text-header-muted border-b border-[#EC67A1]/10 mb-1">
                                <div
                                  className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${isOwned ? "bg-gradient-to-br from-[#EC67A1]/50 to-[#F774B9]/50" : "bg-gradient-to-br from-amber-500/50 to-orange-500/50"}`}
                                >
                ...
                {!isAllProfiles &&
                  visibleFolders.some(
                    (f) => f.parentId !== null && !f.isDefault,
                  ) && (
                    <div
                      onDragOver={handleRootDropZoneDragOver}
                      onDragLeave={handleRootDropZoneDragLeave}
                      onDrop={handleRootDropZoneDrop}
                      className={`mt-2 mb-3 p-3 border-2 border-dashed rounded-xl transition-all ${
                        isRootDropZoneActive
                          ? "border-[#5DC3F8] bg-[#5DC3F8]/10"
                          : "border-[#EC67A1]/20 hover:border-[#EC67A1]/40"
                      }`}
                    >
                      <div className="flex items-center gap-2 text-xs text-center justify-center">
                        ...
                        <span
                          className={
                            isRootDropZoneActive
                              ? "text-[#5DC3F8] font-medium"
                              : "text-header-muted"
                          }
                        >
                          {isRootDropZoneActive
                            ? "Drop to move to root"
                            : "Drag folder here to move to root"}
                        </span>
                      </div>
                    </div>
                  )}
                ...
                {sharedFolders.length > 0 && (
                  <div className="mt-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs bg-[#EC67A1]/20 text-[#EC67A1] px-2 py-0.5 rounded-full">
                    ...
                    <div className="space-y-1">
                      {sharedFolders.map((shared) => {
                        const isActive =
                          selectedSharedFolder?.folderId === shared.folderId &&
                          adminViewMode === "personal";
                        return (
                          <button
                            key={shared.id}
                            onClick={() => {
                              loadSharedFolderItems(shared);
                              setSidebarOpen(false);
                              setAdminViewMode("personal");
                            }}
                            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${isActive ? "bg-gradient-to-r from-[#EC67A1]/20 to-[#F774B9]/20 text-[#EC67A1] border border-[#EC67A1]/30" : "hover:bg-zinc-100 dark:hover:bg-zinc-800 text-header-muted border border-transparent"}`}
                          >
                            <Users
                              className={`w-4 h-4 flex-shrink-0 ${isActive ? "text-[#EC67A1]" : "text-header-muted"}`}
                            />
                            ...
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${shared.permission === "EDIT" ? "bg-emerald-500/20 text-emerald-400" : "bg-zinc-200 dark:bg-zinc-700 text-header-muted"}`}
                            >
                              {shared.permission === "EDIT" ? "Edit" : "View"}
                            </span>
                          </button>
                        );
                      })}
            ...
            {isAdmin && adminViewMode === "creators" && (
              <div className="space-y-4">
                <div className="text-xs font-semibold text-header-muted uppercase tracking-wider mb-3">
                  Creator Stats
                </div>
                {selectedContentCreator ? (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-2 bg-zinc-200 dark:bg-zinc-700/50 rounded-lg text-center">
                        <p className="text-lg font-semibold text-sidebar-foreground">
                          {
                            contentCreatorItems.filter(
                              (i) => i.creatorId === selectedContentCreator.id,
                            ).length
                          }
                        </p>
                        ...
                        <p className="text-lg font-semibold text-sidebar-foreground">
                          {formatFileSize(
                            contentCreatorItems
                              .filter(
                                (i) =>
                                  i.creatorId === selectedContentCreator.id,
                              )
                              .reduce((acc, i) => acc + i.fileSize, 0),
                          )}
                        </p>
                  ...
                  <div className="p-4 bg-zinc-100 dark:bg-zinc-800/50 rounded-xl border border-[#EC67A1]/10">
                    <p className="text-sm text-header-muted mb-2">
                      All Creators
                    </p>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-2 bg-zinc-200 dark:bg-zinc-700/50 rounded-lg text-center">
                      ...
                      <div className="p-2 bg-zinc-200 dark:bg-zinc-700/50 rounded-lg text-center">
                        <p className="text-lg font-semibold text-sidebar-foreground">
                          {formatFileSize(creatorTotalSize)}
                        </p>
                        <p className="text-xs text-header-muted">Total Size</p>
                      </div>
                    </div>
                    <div className="mt-3 flex items-center gap-3 text-xs text-header-muted">
                      <span className="flex items-center gap-1">
                        <ImageIcon className="w-3 h-3" /> {creatorImageCount}
                      </span>
                      <span className="flex items-center gap-1">
                        <VideoIcon className="w-3 h-3" /> {creatorVideoCount}
                      </span>
                      <span className="flex items-center gap-1">
                        <Users className="w-3 h-3" /> {contentCreators.length}{" "}
                        creators
                      </span>
                    </div>
                  </div>
                ...
                {selectedContentCreator && creatorFolders.length > 0 && (
                  <div className="mt-4">
                    ...
                    >
                      <Folder
                        className={`w-4 h-4 ${!selectedCreatorFolderId ? "text-[#EC67A1]" : "text-header-muted"}`}
                      />
                      <span className="flex-1 text-sm font-medium text-left">
                        All Items
                      </span>
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${!selectedCreatorFolderId ? "bg-[#EC67A1]/30 text-[#EC67A1]" : "bg-zinc-200 dark:bg-zinc-700 text-header-muted"}`}
                      >
                        {
                          contentCreatorItems.filter(
                            (i) => i.creatorId === selectedContentCreator.id,
                    ...
                    {creatorProfiles.map((profile) => {
                      const profileFolders = creatorFolders.filter(
                        (f) => f.profileId === profile.id,
                      );
                      if (profileFolders.length === 0) return null;
                        ...
                        <div key={profile.id} className="mb-3">
                          ...
                          <div className="space-y-1">
                            {profileFolders.map((folder) => {
                              const isActive =
                                selectedCreatorFolderId === folder.id;
                              return (
                                <button
                                  key={folder.id}
                                  onClick={() => {
                                    setSelectedCreatorFolderId(folder.id);
                                    setSidebarOpen(false);
                                  }}
                                ...
                                >
                                  ...
                                  <span
                                    className={`text-xs px-2 py-0.5 rounded-full ${isActive ? "bg-[#EC67A1]/30 text-[#EC67A1]" : "bg-zinc-200 dark:bg-zinc-700 text-header-muted"}`}
                                  >
                                    {folder.itemCount}
                                  </span>
                                </button>
          ...
          {adminViewMode === "personal" && selectedProfileId && (
            <div className="p-4 border-t border-[#EC67A1]/10">
              ...
              <div className="mt-2 flex items-center gap-4 text-xs text-header-muted">
                ...
                <span className="flex items-center gap-1">
                  <VideoIcon className="w-3 h-3" /> {videoCount}
                </span>
                <span className="flex items-center gap-1">
                  <FileIcon className="w-3 h-3" /> {totalItems}
                </span>
                {isAllProfiles && (
                  <span className="flex items-center gap-1">
                    <Users className="w-3 h-3" /> {profiles.length} profiles
                  </span>
                )}
              </div>
            </div>
          ...
          <div
            className={`hidden lg:block absolute top-0 right-0 w-1.5 h-full cursor-col-resize group z-10 ${
              isResizingSidebar
                ? "bg-[#EC67A1]/50"
                : "bg-transparent hover:bg-[#EC67A1]/30"
            }`}
          ...
          >
            ...
            <div
              className={`absolute top-1/2 -translate-y-1/2 left-0 w-full h-20 transition-all ${
                isResizingSidebar
            ...
            {/* Grip dots indicator */}
            <div
              className={`absolute top-1/2 -translate-y-1/2 left-0.5 flex flex-col gap-1 transition-opacity ${
                isResizingSidebar
                  ? "opacity-100"
          ...
          <div className="bg-zinc-50/95 dark:bg-[#1a1625]/95 backdrop-blur border-b border-[#EC67A1]/10 px-4 sm:px-6 py-3 sm:py-4">
            ...
            <div className="flex items-center gap-3 lg:hidden mb-3">
              <button
                ...
                className="p-2 bg-zinc-200 dark:bg-zinc-700 hover:bg-zinc-300 dark:hover:bg-zinc-600 rounded-xl transition-colors"
              >
              ...
              {!isViewingShared && !isViewingCreators && (
                <button
                  ...
                  className="p-2 bg-gradient-to-r from-[#EC67A1] to-[#F774B9] text-white rounded-xl transition-colors disabled:opacity-50 shadow-lg shadow-[#EC67A1]/25"
                  title={
                    selectedProfileId === "all"
                      ? "Select a specific profile to upload"
                      : undefined
                  }
                >
              ...
              {vaultItems.filter((item) => item.fileType.startsWith("image/"))
                .length > 0 && (
                <button
              ...
              <div className="flex-shrink min-w-0 max-w-[30%] xl:max-w-none">
                ...
                {isViewingShared && selectedSharedFolder && (
                  <p className="text-xs xl:text-sm text-header-muted flex items-center gap-1 mt-0.5 hidden xl:flex">
                    ...
                    {!canEdit && (
                      <span className="text-amber-500 ml-2">(View only)</span>
                    )}
                  </p>
                )}
                {isViewingCreators && (
              ...
              <div className="flex items-center gap-1 xl:gap-3 flex-shrink-0">
                <div className="relative w-24 xl:w-64">
                  ...
                  <input
                    ...
                    className="w-full pl-7 xl:pl-9 pr-2 xl:pr-4 py-1.5 xl:py-2.5 bg-zinc-100 dark:bg-zinc-800 border border-[#EC67A1]/20 rounded-lg xl:rounded-xl text-xs xl:text-sm text-sidebar-foreground placeholder-header-muted focus:ring-2 focus:ring-[#EC67A1] focus:border-[#EC67A1] transition-colors"
                  />
                </div>
                <div className="flex items-center bg-zinc-100 dark:bg-zinc-800 rounded-lg xl:rounded-xl p-0.5 xl:p-1 border border-[#EC67A1]/20">
                  <button
                    ...
                    className={`p-1.5 xl:p-2 rounded-lg transition-colors ${viewMode === "grid" ? "bg-gradient-to-r from-[#EC67A1] to-[#F774B9] text-white shadow-lg shadow-[#EC67A1]/25" : "text-header-muted hover:text-sidebar-foreground"}`}
                  >
                    <Grid3X3 className="w-3.5 xl:w-4 h-3.5 xl:h-4" />
                  </button>
                  <button
                    onClick={() => setViewMode("list")}
                    className={`p-1.5 xl:p-2 rounded-lg transition-colors ${viewMode === "list" ? "bg-gradient-to-r from-[#EC67A1] to-[#F774B9] text-white shadow-lg shadow-[#EC67A1]/25" : "text-header-muted hover:text-sidebar-foreground"}`}
                  >
                    <List className="w-3.5 xl:w-4 h-3.5 xl:h-4" />
                  </button>
                </div>
                {!isViewingShared && !isViewingCreators && (
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 w-9 h-9 xl:w-auto xl:h-auto xl:px-4 xl:py-2.5 bg-gradient-to-r from-[#EC67A1] to-[#F774B9] hover:from-[#E1518E] hover:to-[#EC67A1] text-white rounded-lg xl:rounded-xl font-medium transition-all disabled:opacity-50 shadow-lg shadow-[#EC67A1]/25"
                    title="Upload"
                  >
                    <Upload className="w-4 h-4" />{" "}
                ...
                {!isViewingShared && !isViewingCreators && (
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 w-9 h-9 xl:w-auto xl:h-auto xl:px-4 xl:py-2.5 bg-gradient-to-r from-[#5DC3F8] to-[#4ab3e8] hover:from-[#4ab3e8] hover:to-[#3aa3d8] text-white rounded-lg xl:rounded-xl font-medium transition-all disabled:opacity-50 shadow-lg shadow-[#5DC3F8]/25"
                    title="Google Drive"
                  >
                    <HardDrive className="w-4 h-4" />{" "}
            ...
            <div className="flex items-center gap-2 lg:hidden mb-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-9 pr-4 py-2 bg-zinc-100 dark:bg-zinc-800 border border-[#EC67A1]/20 rounded-xl text-sm text-sidebar-foreground placeholder-header-muted focus:ring-2 focus:ring-[#EC67A1] focus:border-[#EC67A1] transition-colors"
                />
              </div>
              <div className="flex items-center bg-zinc-100 dark:bg-zinc-800 rounded-xl p-1 border border-[#EC67A1]/20">
                <button
                  onClick={() => setViewMode("grid")}
                  className={`p-2 rounded-lg transition-colors ${viewMode === "grid" ? "bg-gradient-to-r from-[#EC67A1] to-[#F774B9] text-white" : "text-header-muted hover:text-sidebar-foreground"}`}
                >
                  <Grid3X3 className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setViewMode("list")}
                  className={`p-2 rounded-lg transition-colors ${viewMode === "list" ? "bg-gradient-to-r from-[#EC67A1] to-[#F774B9] text-white" : "text-header-muted hover:text-sidebar-foreground"}`}
                >
              ...
              <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto pb-1 sm:pb-0 -mx-1 px-1 sm:mx-0 sm:px-0 vault-scroll">
                {["all", "photos", "videos", "audio", "gifs"].map((filter) => (
                  <button
                    ...
                    className={`px-3 sm:px-4 py-1.5 text-xs sm:text-sm font-medium rounded-xl transition-all whitespace-nowrap flex-shrink-0 ${contentFilter === filter ? "bg-gradient-to-r from-[#EC67A1] to-[#F774B9] text-white shadow-lg shadow-[#EC67A1]/25" : "text-header-muted hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-sidebar-foreground"}`}
                  >
            ...
            <VaultEnhancements
              ...
              onSearchInMetadataToggle={() =>
                setSearchInMetadata(!searchInMetadata)
              }
              ...
              <div className={getGridClasses}>
                {[...Array(8)].map((_, i) => (
                  <div
                    ...
                    className={
                      viewMode === "grid"
                        ? "bg-zinc-100 dark:bg-zinc-800/50 border border-[#EC67A1]/10 rounded-xl p-2 sm:p-3 animate-pulse"
                        : "bg-zinc-100 dark:bg-zinc-800/50 border border-[#EC67A1]/10 rounded-xl p-3 animate-pulse flex items-center gap-3 sm:gap-4"
                    }
                  >
                    <div
                      className={
                        viewMode === "grid"
                          ? "aspect-square bg-zinc-200 dark:bg-zinc-700 rounded-lg mb-2 sm:mb-3"
                          : "w-10 sm:w-12 h-10 sm:h-12 bg-zinc-200 dark:bg-zinc-700 rounded-lg flex-shrink-0"
                      }
                    />
                    <div className={viewMode === "grid" ? "" : "flex-1"}>
                      <div className="h-3 sm:h-4 bg-zinc-200 dark:bg-zinc-700 rounded w-3/4 mb-2" />
                      <div className="h-2 sm:h-3 bg-zinc-200 dark:bg-zinc-700 rounded w-1/2" />
                    </div>
                  </div>
                ))}
              </div>
            ...
            !selectedSharedFolder ? (
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                <div className="w-16 h-16 bg-gradient-to-br from-[#EC67A1]/20 to-[#F774B9]/20 rounded-2xl flex items-center justify-center mb-4 border border-[#EC67A1]/30">
                  <Folder className="w-8 h-8 text-[#EC67A1]" />
                </div>
                <h3 className="text-lg font-medium text-sidebar-foreground mb-1">
                  Select a profile
                </h3>
                <p className="text-sm text-header-muted">
                  Choose a profile from the sidebar to view your files
                </p>
                <button
                  onClick={() => setSidebarOpen(true)}
                  className="mt-4 lg:hidden flex items-center gap-2 px-4 py-2.5 bg-zinc-200 dark:bg-zinc-700 hover:bg-zinc-300 dark:hover:bg-zinc-600 text-header-muted rounded-xl font-medium transition-colors"
                >
              ...
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                <div className="w-16 h-16 bg-gradient-to-br from-[#EC67A1]/20 to-[#F774B9]/20 rounded-2xl flex items-center justify-center mb-4 border border-[#EC67A1]/30">
                  <FolderPlus className="w-8 h-8 text-[#EC67A1]" />
                </div>
                <h3 className="text-lg font-medium text-sidebar-foreground mb-1">
                  Create a folder
                </h3>
              ...
              <div className="flex flex-col items-center justify-center h-full text-center px-4">
                ...
                <p className="text-sm text-header-muted mb-4">
                  {isAllProfiles
                    ? "Select a specific profile to upload files"
                    : "Upload some files to get started"}
                </p>
                {!isViewingShared && !isViewingCreators && !isAllProfiles && (
                  <button
                    onClick={() => setIsAddingNew(true)}
                    className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#EC67A1] to-[#F774B9] hover:from-[#E1518E] hover:to-[#EC67A1] text-white rounded-xl font-medium transition-all shadow-lg shadow-[#EC67A1]/25"
                  >
                  ...
                  <div className="flex items-center justify-between mb-2 px-3 py-2.5 bg-zinc-100 dark:bg-zinc-800/50 border border-[#EC67A1]/10 rounded-xl">
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-4 h-4 text-[#EC67A1] bg-zinc-100 dark:bg-zinc-800 border-[#EC67A1]/30 rounded focus:ring-[#EC67A1] cursor-pointer"
                      />
                      <span className="text-xs sm:text-sm text-sidebar-foreground font-medium">
                        {selectedItems.size > 0
                          ? `${selectedItems.size} of ${currentFilteredItems.length}`
                          : "Select all"}
                      </span>
                    </div>
            ...
            <div className="fixed sm:absolute bottom-20 sm:bottom-4 left-1/2 -translate-x-1/2 z-30 animate-slideUp w-[calc(100%-2rem)] sm:w-auto max-w-2xl">
              <div className="flex items-center gap-1.5 sm:gap-3 bg-zinc-100/95 dark:bg-[#1a1625]/95 backdrop-blur-xl border border-[#EC67A1]/20 rounded-2xl px-2.5 sm:px-5 py-2.5 sm:py-3 shadow-2xl shadow-[#EC67A1]/10">
                ...
                <div className="flex items-center gap-2 pr-2 sm:pr-3 border-r border-[#EC67A1]/20">
                  ...
                  <span className="text-xs sm:text-sm text-header-muted hidden sm:inline">
                    selected
                  </span>
                </div>
                ...
                {googleDriveImportSuccess ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-header-muted">
                      ...
                      {googleDriveImportSuccess.itemCount !== 1 ? "s" : ""}{" "}
                      imported to your vault
                    </p>
                  </div>
                ) : !googleDriveAccessToken ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-header-muted mb-6 max-w-md mx-auto">
                      To import files from Google Drive, you need to connect
                      your account first.
                    </p>
                    <button
                    ...
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center gap-2 text-sidebar-foreground">
                        ...
                        <span className="font-medium">
                          Paste a Google Drive folder link to browse
                        </span>
                      </div>
                      <div className="flex gap-2 p-4 bg-zinc-100/50 dark:bg-zinc-800/30 rounded-xl border border-[#EC67A1]/10">
                        <div className="flex-1 relative">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setGoogleDriveLinkInput(e.target.value)
                            }
                            onKeyDown={(e) => {
                              if (
                                e.key === "Enter" &&
                    ...
                    <div className="flex items-center gap-1 text-sm bg-zinc-100/50 dark:bg-zinc-800/30 rounded-xl px-4 py-2.5 overflow-x-auto">
                      {googleDriveBreadcrumbs.map((crumb, index) => (
                        <div key={index} className="flex items-center">
                          {index > 0 && (
                            <ChevronRight className="w-4 h-4 text-header-muted mx-1" />
                          )}
                          <button
                      ...
                      <div className="ml-auto flex items-center gap-2">
                        <button
                          onClick={() =>
                            fetchGoogleDriveContents(currentGoogleDriveFolderId)
                          }
                        ...
                        >
                          <RefreshCw
                            className={`w-4 h-4 ${loadingGoogleDriveFiles ? "animate-spin" : ""}`}
                          />
                        </button>
                        <button
                          onClick={() => {
                      ...
                      <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-xl text-red-500 text-sm flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-medium">
                            {googleDriveError.includes("permission") ||
                            googleDriveError.includes("access")
                              ? "Access Denied"
                              : "Error"}
                          </p>
                        ...
                        <div>
                          ...
                          {googleDriveFiles.length === 0 ? (
                            <div className="text-center py-12 text-header-muted bg-zinc-100/50 dark:bg-zinc-800/30 rounded-xl">
                              ...
                              <p className="text-sm mt-1 text-header-muted">
                                Browse into folders to find images, videos, and
                                audio files
                              </p>
                            </div>
                          ) : (
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 max-h-[320px] overflow-y-auto pr-2">
                              {googleDriveFiles.map((file) => (
                                <div
                                  key={file.id}
                                  onClick={() =>
                                    toggleGoogleDriveFileSelection(file.id)
                                  }
                ...
                <div className="p-6 border-t border-[#EC67A1]/10 flex gap-3 shrink-0">
                  ...
                  >
                    {importingFromGoogleDrive ? (
                      ...
                      <>
                        <HardDrive className="w-4 h-4" />
                        Import {selectedGoogleDriveFiles.size} File
                        {selectedGoogleDriveFiles.size !== 1 ? "s" : ""}
                      </>
                    )}
                  </button>
                </div>
      ...
      <PlatformExportModal
        ...
        onExportComplete={(result) => {
          showToast(
            `Exported ${result.fileCount} files to ${result.filename}`,
            "success",
          );
      ...
      {contextMenuFolder &&
        ...
        createPortal(
          <div
            className="fixed z-[200] bg-white dark:bg-[#1a1625] rounded-xl shadow-2xl border border-[#EC67A1]/20 py-1 min-w-[180px] animate-fadeIn"
            style={{
              left: `${Math.min(contextMenuPosition.x, window.innerWidth - 200)}px`,
              top: `${Math.min(contextMenuPosition.y, window.innerHeight - 250)}px`,
            }}
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleOpenShareModal(contextMenuFolder);
                closeContextMenu();
              }}
              className="w-full flex items-center gap-3 px-4 py-2 text-sm text-sidebar-foreground hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
            >
              <Share2 className="w-4 h-4 text-header-muted" />
              <span>Share</span>
            </button>
            <button
              onClick={(e) => {
                e.stopPropagation();
                startMoveFolder(contextMenuFolder);
                closeContextMenu();
              }}
              className="w-full flex items-center gap-3 px-4 py-2 text-sm text-sidebar-foreground hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
            >
              <Move className="w-4 h-4 text-header-muted" />
              <span>Move folder</span>
            </button>
            <button
              onClick={(e) => {
                e.stopPropagation();
                startEditFolder(contextMenuFolder);
                closeContextMenu();
              }}
              className="w-full flex items-center gap-3 px-4 py-2 text-sm text-sidebar-foreground hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
            >
              <Edit2 className="w-4 h-4 text-header-muted" />
              <span>Rename</span>
            </button>
            <div className="h-px bg-[#EC67A1]/10 my-1" />
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleDeleteFolder(contextMenuFolder.id);
                closeContextMenu();
              }}
              className="w-full flex items-center gap-3 px-4 py-2 text-sm text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"
            >
              <Trash2 className="w-4 h-4" />
              <span>Delete</span>
            </button>
