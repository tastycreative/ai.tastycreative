...
def run_training_process(job_input, job_id, webhook_url):
    ...
    try:
        ...
        logger.info("🚀 About to send 'Processing training images...' webhook")
        send_webhook(webhook_url, {
            'job_id': job_id,
            'status': 'IN_PROGRESS',
            'progress': 10,
        ...
        logger.info("🚀 Webhook sent (async), immediately continuing with image processing")
        ...
        for idx, image_info in enumerate(job_input['imageUrls']):
            logger.info(f"🔄 Loop iteration {idx + 1}/{len(job_input['imageUrls'])}")
            try:
                ...
                logger.info(f"✅ Saved image: {image_path} ({len(response.content) if hasattr(response, 'content') else 'streamed'} bytes)")
                image_count += 1
...
def handler(job):
    """Enhanced RunPod handler for serverless execution"""
    job_input = job['input']
    ...
    try:
        ...
        logger.info(f"🎯 Starting synchronous training process for serverless job: {job_id}")
